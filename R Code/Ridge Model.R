...
indep_var <- indep_var[c("STA_PF","Opp_KP_ADE_Rk","Opp_KP_Non.Conference_Strength_of_Schedule","KP_Ajusted_Efficiency_Margin","TS_3PA","KP_Non.Conference_Strength_of_Schedule","AP_Rnk","KP_AOOE_Rk","Opp_KP_AOOE_Rk","STA_Top25_L","Opp_AP_Rnk","Opp_STA_Home_L","Opp_STA_Top25_W","Opp_KP_AOW_Rk","Opp_KP_Luck_Rk","KP_AODE_Rk","KP_AT_Rk","Opp_KP_AT_Rk","KP_AOE_Rk","Opp_STA_Neutral_W")]

indep_var_mat = as.matrix(indep_var)
lambdas <- 10^seq(-4, 2, by = 0.01)

ridge_reg <- cv.glmnet(indep_var_mat, dep_var, alpha = 0, lambda = lambdas, standardize = FALSE)

# Best 
lambda_best <- ridge_reg$lambda.min 
lambda_best

ridge_model <- glmnet(indep_var_mat, dep_var, alpha = 0, lambda = lambda_best, standardize = FALSE)
coef(ridge_model)
...
pred <- predict(ridge_model, s = lambda_best, newx =bracket_indep_var_mat)
...
for(i in 1:nrow(act_scores)){
  mse_ridge = mse_ridge + (act_scores[i,2]-pred[i,1])^2
}
mse_ridge
